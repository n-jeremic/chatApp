...
const currentUser = JSON.parse(
  document.getElementById('currentUser').dataset.currentUser
);
  ...
  try {
    ...
    if (response.data.data.chat === null) {
      ...
      openChatButton(
        ...
        user_name,
        user_photo,
        response.data.data.chat.messages,
        newMessage
      );
    }
  } catch (err) {
...
const openChatButton = (
  ...
  if (messages != null) {
    messages.forEach(msg => {
      if (msg.to.id != user_id) {
        markUpChat += createChatMessage(msg.text, user_photo, 'notMe');
      } else {
        markUpChat += createChatMessage(msg.text, null, 'me');
      }
    });
  }
  ...
  const markUpButton = `<div class="col-lg-4" id="entireChat-${user_id}"><p><button
    class="btn btn-${
      newMessage == 'false' ? 'light' : 'primary'
    } btn-block btn-lg"
    type="button"
    id="btnChat-${user_id}"
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        let message = createChatMessage(
          response.data.data.message.text,
          null,
          'me'
        );
...
const createChatMessage = (message, user_photo, from) => {
  let markUpChat = '';
  if (from === 'me') {
    markUpChat += `<div class="row"><div class="col-lg-12 no-padding"><p class="float-left">
          ...
          width="40px"
          class="img-fluid rounded" /><span class="text-message align-middle ml-2" style="background-color: white">${message}</span></p></div></div>`;
  } else {
    markUpChat += `<div class="row"><div class="col-lg-12 no-padding"><p class="float-right">
        <span class="text-message align-middle mr-2" style="background-color: #ffff80">${message}</span><img
        src="/img/users/${user_photo}"
        width="40px"
        class="img-fluid rounded" /></p></div></div>`;
  }
...
async function getNewMessages() {
  try {
    ...
    for (let i = 0; i < newMsgsArr.length; i++) {
      const openedChat = document.getElementById(
        `btnChat-${newMsgsArr[i].from.id}`
      );
      ...
      if (openedChat) {
        ...
        document
          .querySelector(`#btnChat-${newMsgsArr[i].from.id}`)
        ...
        const user_name =
          newMsgsArr[i].from.firstName + ' ' + newMsgsArr[i].from.lastName;
        await getChat(
